<!--
Historical changelog, most recent release first. Unlike RELEASE_NOTES.md
(which accumulates unreleased changes and resets after each release), this
file is never cleared: `Scripts/release.sh` prepends the "Unreleased"
content here, dated and under the new version number, right before
resetting RELEASE_NOTES.md.

Bundled inside the app (see Scripts/bundle-app.sh) and shown by SmartMeet's
"What's New" window — always in English, regardless of the app's own
localized UI, since these notes are only ever written in English.
-->
